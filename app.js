...
async function getWeatherData(city) {
  ...
  const response = await fetch(
    "http://api.weatherapi.com/v1/forecast.json?key=207599b971564ec4905144139242606&q=" +
      searchedCity +
  ...
  localTime.innerHTML =
    "Local Time: " +
    weatherData.location.localtime.slice(
      weatherData.location.localtime.length - 5,
      weatherData.location.localtime.length
    );
...
async function getTwentyFourHourForecast(weatherData) {
  ...
  while (twentyFourHourForecastData.firstChild) {
    twentyFourHourForecastData.removeChild(
      twentyFourHourForecastData.firstChild
    );
  }
